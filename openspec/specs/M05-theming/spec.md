...
Alignment with the cross-platform Farpost style guide, so `farpost-mobile` reads
as the same product as `farpost-web` rather than a visually distinct app that
happens to share a backend. Backfilled 2026-07-17 from the M01 skeleton commit; no
code changed to produce this spec.
